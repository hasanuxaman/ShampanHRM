...
namespace SymWebUI.Areas.HRM.Controllers
{
    ...
    public class EmployeeLeaveController : Controller
    {
        ...
        public JsonResult DayCountSkipHoliday(string employeeId,string FromDate, string ToDate)
        {
            try
            {
                string HolidayCheck = new SettingDAL().settingValue("Leave", "HolidayCheck");
                ...
                {
                    var employeeJob = new EmployeeJobRepo().SelectByEmployee(employeeId);
                    List<DateTime> businessDays = Ordinary.GetBusinessDaysDateWithoutHollyday(Convert.ToDateTime(FromDate), Convert.ToDateTime(ToDate));

                    return Json(businessDays.Count(), JsonRequestBehavior.AllowGet);
                }
