...
{
    [Authorize]
    public class EmployeeCompensatoryLeaveController : Controller
    {
        //
        // GET: /HRM/EmployeeLeave/
        EmployeeCompensatoryLeaveRepo elrepo = new EmployeeCompensatoryLeaveRepo();
        ...
        public ActionResult _index(JQueryDataTableParamModel param, string status, string empId)
        {
            ...
            List<EmployeeCompensatoryLeaveVM> getAllData = new List<EmployeeCompensatoryLeaveVM>();
            ShampanIdentity Identit = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
            if (identity.IsAdmin || identity.IsHRM)
            {
                getAllData = elrepo.SelectAll();
            }
            else
            {
                getAllData = elrepo.SelectAll(Identit.EmployeeId, "");
            }
            IEnumerable<EmployeeCompensatoryLeaveVM> filteredData;
            ...
            Func<EmployeeCompensatoryLeaveVM, string> orderingFunction = (c => sortColumnIndex == 1 && isSortable_1 ? c.EmpCode :
        ...
        public ActionResult _indexByEmployeeId(JQueryDataTableParamModel param, string empId)
        {
            ...
            List<EmployeeCompensatoryLeaveVM> getAllData = new List<EmployeeCompensatoryLeaveVM>();
            ShampanIdentity Identit = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
            getAllData = elrepo.SelectByEmployeeId(empId);
            IEnumerable<EmployeeCompensatoryLeaveVM> filteredData;
            if (!string.IsNullOrEmpty(param.sSearch))
            {
                var isSearchable1 = Convert.ToBoolean(Request["bSearchable_1"]);
        ...
        public ActionResult Leave(string empleaveId = null)
        {
            ...
            {
                vm = elrepo.SelectById(Convert.ToInt32(empleaveId));
        ...
        public ActionResult LeaveApprove(string parm, string Code, string Id = "", string IsSupervisor = "")
        {
            ...
            try
            {
                ...
                //if (paramStatus == "approve" && result[0] == "Success")
                //{
        ...
        public ActionResult LeaveEdit(string parm, string IsSupervisor = "")
        {
            ...
            EmployeeCompensatoryLeaveRepo repoel = new EmployeeCompensatoryLeaveRepo();
            EmployeeInfoRepo repo = new EmployeeInfoRepo();
            EmployeeCompensatoryLeaveVM EmployeeCompensatoryLeaveVM = repoel.SelectById(Convert.ToInt32(parm.Split('~')[0]));
            if (EmployeeCompensatoryLeaveVM.IsApprove)
            {
                var mgs = "Fail~This Leave already Approved, After Approved leave not editable";
                Session["mgs"] = "mgs";
                return RedirectToAction("Leave", new { mgs = mgs });
            }
            EmployeeInfoVM vm = repo.SelectById(EmployeeCompensatoryLeaveVM.EmployeeId);
            ...
            EmployeeCompensatoryLeaveRepo _levRepo = new EmployeeCompensatoryLeaveRepo();
            List<EmployeeLeaveBalanceVM> employeeLeaveBalanceVMs = _levRepo.EmployeeLeaveBalance(EmployeeCompensatoryLeaveVM.EmployeeId, Session["SessionYear"].ToString());
            ...
            EmployeeCompensatoryLeaveVM.RedirectPage = "EmployeeLeave";
            vm.empCompensatoryleavevm = EmployeeCompensatoryLeaveVM;
            vm.employeeLeaveBalanceVMs = employeeLeaveBalanceVMs;
            return View(vm);
        }

        [HttpPost]
        public ActionResult CompensatoryLeave(EmployeeCompensatoryLeaveVM vm)
        {
            ShampanIdentity identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
            ...
            EmployeeCompensatoryLeaveRepo repo = new EmployeeCompensatoryLeaveRepo();
            vm.CreatedAt = DateTime.Now.ToString("yyyyMMddHHmmss");
            vm.LeaveYear = Convert.ToInt32(Convert.ToDateTime(vm.FromDate).ToString("yyyy"));
            vm.CreatedBy = identity.Name;
            vm.CreatedFrom = identity.WorkStationIP;
            vm.LeaveType_E = "Compensatory Leave";
        ...
        public ActionResult EmployeeLeavedetailForSearch(EmployeeCompensatoryLeaveVM vm)
        {
            EmployeeInfoRepo repo = new EmployeeInfoRepo();
            EmployeeCompensatoryLeaveRepo _levRepo = new EmployeeCompensatoryLeaveRepo();
            EmployeeInfoVM varEmployeeInfoVM = new EmployeeInfoVM();
            ////EmployeeCompensatoryLeaveVM empCompensatoryleavevm = new EmployeeCompensatoryLeaveVM();
            //EmployeeCompensatoryLeaveVM EmployeeCompensatoryLeaveVM = repoel.SelectById(Convert.ToInt32(parm.Split('~')[0]));
            if (vm.LeaveYear == 0)
            {
            ...
            varEmployeeInfoVM.empCompensatoryleavevm = vm;
            if (vm.Id > 0)
            {
                varEmployeeInfoVM.empCompensatoryleavevm = _levRepo.SelectById(Convert.ToInt32(vm.Id));
            }
            varEmployeeInfoVM.empCompensatoryleavevm.RedirectPage = vm.RedirectPage;
        ...
        [HttpPost]
        public ActionResult LeaveEdit(EmployeeCompensatoryLeaveVM vm)
        {
            #region Objects and Variables
            ShampanIdentity identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
        ...
        public ActionResult EmployeeLeavedetaillistForSearch(int? Id, string empcode = "", string btn = "current")
        {
            EmployeeInfoRepo repo = new EmployeeInfoRepo();
            EmployeeInfoVM vm = new EmployeeInfoVM();
            EmployeeCompensatoryLeaveVM empleave = new EmployeeCompensatoryLeaveVM();
            if (Id != null)
            {
            ...
            vm.empCompensatoryleavevm = empleave;
            return PartialView("_leaveDetailListForSearch", vm);
        }

        public ActionResult LeavedetailForSearch(string id, string empcode = "", string btn = "current", string leaveyear = "0")
        {
            EmployeeInfoRepo repo = new EmployeeInfoRepo();
            EmployeeCompensatoryLeaveRepo _levRepo = new EmployeeCompensatoryLeaveRepo();
            EmployeeInfoVM vm = new EmployeeInfoVM();
        ...
        public void EmpLeaveApplyEmailProcess(string Code = "", string Id = "", string EmployeeId = "")
        {
            ...
            EmployeeCompensatoryLeaveVM empCompensatoryleavevm = new EmployeeCompensatoryLeaveVM();
            empCompensatoryleavevm = elrepo.SelectById(Convert.ToInt32(Id));
            ...
            //else
            //{
                empJobVM = _empJobRepo.SelectByEmployee(empCompensatoryleavevm.EmployeeId);
            //}
            ...
            double totalDays = (Convert.ToDateTime(empCompensatoryleavevm.ToDate) - Convert.ToDateTime(empCompensatoryleavevm.FromDate)).TotalDays + 1;
            ...
            ems.MailHeader = ems.MailHeader.Replace("vFromDate", empCompensatoryleavevm.FromDate);
            ems.MailHeader = ems.MailHeader.Replace("vToDate", empCompensatoryleavevm.ToDate);
            ...
            mailbody = mailbody.Replace("vFromDate", empCompensatoryleavevm.FromDate);
            mailbody = mailbody.Replace("vToDate", empCompensatoryleavevm.ToDate);
            mailbody = mailbody.Replace("vTotalDays", totalDays.ToString());
            mailbody = mailbody.Replace("vLeaveType", empCompensatoryleavevm.LeaveType_E);
            mailbody = mailbody.Replace("vPurpose", empCompensatoryleavevm.Remarks);
            mailbody = mailbody.Replace("vEmpName", empCompensatoryleavevm.EmpName);
            ...
            foreach (var item in getAllData)
            {
                try
                {
                    ...
                    //ems.MailToAddress = "khalid.sarower@symphonysoftt.com";
                    if (!string.IsNullOrWhiteSpace(ems.MailToAddress))
                    {

                        ems.MailBody = mailbody;
                        ems.FileName = empCompensatoryleavevm.EmpName + " (" + empCompensatoryleavevm.FromDate + "-" + empCompensatoryleavevm.ToDate + ")";
        ...
        public void EmpLeaveApproveEmailProcess(string Code = "", string Id = "", string EmployeeId = "")
        {
            ...
            try
            {
                ...
                double totalDays = (Convert.ToDateTime(vm.ToDate) - Convert.ToDateTime(vm.FromDate)).TotalDays + 1;
                #endregion
