...
{
    public class EmployeeAreerController : Controller
    {
        //
        // GET: /Payroll/EmployeeAreer/

        EmployeeAreerRepo _eaRepo;

        #region Action Methods

        public ActionResult Index()
        {
            //return View(new EmployeeAreerRepo().SelectAll());
            return View();
        }
        public ActionResult _index(JQueryDataTableParamModel param, string code, string name)
        {
            _eaRepo = new EmployeeAreerRepo();
            ...
            var areerAmountFilter = Convert.ToString(Request["sSearch_5"]);
            ...
            {
                amountFrom = areerAmountFilter.Split('~')[0] == "" ? 0 : Ordinary.IsInteger(areerAmountFilter.Split('~')[0]) == true ? Convert.ToInt32(areerAmountFilter.Split('~')[0]) : 0;
                amountTo = areerAmountFilter.Split('~')[1] == "" ? 0 : Ordinary.IsInteger(areerAmountFilter.Split('~')[1]) == true ? Convert.ToInt32(areerAmountFilter.Split('~')[1]) : 0;
            }
            ...
            IEnumerable<EmployeeAreerVM> filteredData;
            if (!string.IsNullOrEmpty(param.sSearch))
            {
                ...
                filteredData = getAllData
                   .Where(c => isSearchable1 && c.Code.ToLower().Contains(param.sSearch.ToLower())
                       ...
                       || isSearchable6 && c.AreerAmount.ToString().Contains(param.sSearch.ToLower())
            ...
            if (codeFilter != "" || empNameFilter != "" || departmentFilter != "" || designationFilter != "" || (areerAmountFilter != "" && areerAmountFilter != "~") || periodNameFilter != "")  
            {
                filteredData = filteredData
                                .Where(c => (codeFilter == "" || c.Code.ToLower().Contains(codeFilter.ToLower()))
                                    ...
                                    &&
                                    (amountFrom == 0 || amountFrom <= Convert.ToInt32(c.AreerAmount))
                                    &&
                                    (amountTo == 0 || amountTo >= Convert.ToInt32(c.AreerAmount))
                                    &&
            ...
            Func<EmployeeAreerVM, string> orderingFunction = (c => sortColumnIndex == 1 && isSortable_1 ? c.Code :
                ...
                sortColumnIndex == 6 && isSortable_6 ? c.AreerAmount.ToString() :
        ...
        [HttpGet]
        public ActionResult EmployeeInfoForAreer()
        {
            return View("_Employee");
        }
        [Authorize]
        public ActionResult _EmployeeInfoForAreer(JQueryDataTableParamModel param, string code, string name)
        {
            #region Column Search
            var idFilter = Convert.ToString(Request["sSearch_0"]);
            var codeFilter = Convert.ToString(Request["sSearch_1"]);
            ...
            List<EmployeeInfoVM> getAllData = _empRepo.SelectAll();
        ...
        public ActionResult Create(EmployeeInfoVM empVM)
        {
            ...
            vm.AreerDate = new FiscalYearRepo().FiscalPeriodStartDate(vm.FiscalYearDetailId);
                if (vm.FiscalYearDetailId == 0)
            {
                ...
                result = new EmployeeAreerRepo().Insert(vm);
        ...
        public ActionResult DetailCreate(string empcode = "", string btn = "current", string FiscalYearDetailId = "0")
        {
            string EmployeeId = "";
            EmployeeInfoRepo repo = new EmployeeInfoRepo();
            EmployeeAreerRepo arerepo = new EmployeeAreerRepo();

            EmployeeAreerVM arrearVM = new EmployeeAreerVM();
            EmployeeInfoVM vm = new EmployeeInfoVM();
	...
	{
        arrearVM = arerepo.SelectByIdandFiscalyearDetail(vm.Id, FiscalYearDetailId);
        arrearVM.FiscalYearDetailId =Convert.ToInt32(FiscalYearDetailId);
            ...
            vm.arrearVM = arrearVM;
            vm.arrearVM.EmployeeId = EmployeeId;
            vm.arrearVM.FiscalYearDetailId =Convert.ToInt32(FiscalYearDetailId);
        ...
        [Authorize(Roles = "Master,Admin,Account")]
        [HttpPost]
        public ActionResult Edit(EmployeeAreerVM vm)
        {
            string[] result = new string[6];
           try
            {
            ...
            vm.LastUpdateFrom = identity.WorkStationIP;
                ...
                result = new EmployeeAreerRepo().Update(vm);
        ...
        [Authorize(Roles = "Master,Admin,Account")]
        public JsonResult EmployeeAreerDelete(string ids)
        {
            EmployeeAreerVM vm = new EmployeeAreerVM();
            ...
            vm.LastUpdateBy = identity.Name;
            vm.LastUpdateFrom = identity.WorkStationIP;
            result = new EmployeeAreerRepo().Delete(vm, a);
            return Json(result[1], JsonRequestBehavior.AllowGet);
        }

        //public ActionResult QCListReport(string mgfOrganizationId, string supplierOrganizationId, string localSupplierOrganizationId, string view)
        //{ 
        //}
        public ActionResult EmployeeAreerListReport(string view, int? fid = null, string ProjectId = null, string DepartmentId = null, string SectionId = null, string DesignationId = null, string CodeF = null, string CodeT = null, string EmpName = null, string dojFrom = null, string dojTo = null)
        {
            try
            {
                ShampanIdentity identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
                string EmployeeAreer = "N";
                ...
                ViewBag.EmployeeAreer = EmployeeAreer;
                if (view == "Y")
                {
                    return View();
                }
                ReportDocument doc = new ReportDocument();
                List<EmployeeAreerVM> getAllData = new List<EmployeeAreerVM>();
                 getAllData = new EmployeeAreerRepo().SelectAll(null, fid);
                ...
                ds.Tables.Add(table);
                ds.Tables[0].TableName = "dtEmpAreer";

                string rptLocation = "";
                rptLocation = AppDomain.CurrentDomain.BaseDirectory + @"AllReports\\Payroll\\PayrollEntry\\rptEmployeeAreer.rpt";
