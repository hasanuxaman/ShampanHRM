...
{
    public class EmployeeReimbursableExpenseController : Controller
    {
        //
        // GET: /Payroll/EmployeeReimbursableExpense/

        EmployeeReimbursableExpenseRepo _eaRepo;
        public ActionResult Index()
        {
            //return View(new EmployeeReimbursableExpenseRepo().SelectAll());
            return View();
        }
        public ActionResult _index(JQueryDataTableParamModel param, string code, string name)
        {
            _eaRepo = new EmployeeReimbursableExpenseRepo();
            ...
            var ReimbursableExpenseAmountFilter = Convert.ToString(Request["sSearch_5"]);
            ...
            {
                amountFrom = ReimbursableExpenseAmountFilter.Split('~')[0] == "" ? 0 : Ordinary.IsInteger(ReimbursableExpenseAmountFilter.Split('~')[0]) == true ?  Convert.ToInt32(ReimbursableExpenseAmountFilter.Split('~')[0]) : 0;
                amountTo = ReimbursableExpenseAmountFilter.Split('~')[1] == "" ? 0 : Ordinary.IsInteger(ReimbursableExpenseAmountFilter.Split('~')[1]) == true ?  Convert.ToInt32(ReimbursableExpenseAmountFilter.Split('~')[1]) : 0;
            ...
            IEnumerable<EmployeeReimbursableExpenseVM> filteredData;
            if (!string.IsNullOrEmpty(param.sSearch))
            {
                ...
                filteredData = getAllData
                   .Where(c => 
                       ...
                       || isSearchable6 && c.ReimbursableExpenseAmount.ToString().Contains(param.sSearch.ToLower())
            ...
            if (codeFilter != "" || empNameFilter != "" || departmentFilter != "" || designationFilter != "" || (ReimbursableExpenseAmountFilter != "" && ReimbursableExpenseAmountFilter != "~") || periodNameFilter != "") 
            {
                filteredData = filteredData
                                .Where(c => 
                                    ...
                                    && 
                                    (amountFrom == 0 || amountFrom <= Convert.ToInt32(c.ReimbursableExpenseAmount))
                                    && 
                                    (amountTo == 0 || amountTo >= Convert.ToInt32(c.ReimbursableExpenseAmount))
                                    &&
            ...
            Func<EmployeeReimbursableExpenseVM, string> orderingFunction = (c => 
                ...
                sortColumnIndex == 6 && isSortable_6 ? c.ReimbursableExpenseAmount.ToString() :
        ...
        [HttpGet]
        public ActionResult EmployeeInfoForReimbursableExpense()
        {
            return View("_Employee");
        }

        [Authorize]
        public ActionResult _EmployeeInfoForReimbursableExpense(JQueryDataTableParamModel param, string code, string name)
        {
        ...
        public ActionResult Create( string empId)
        {
           ...
           vm.ReimbursableExpenseAmount = 0;
           vm.ReimbursableExpenseDate = DateTime.Now.ToString("dd/MMM/yyyy");
           return View(vm);
        }

        [Authorize(Roles = "Master,Admin,Account")]
        [HttpPost]
        public ActionResult Create(EmployeeReimbursableExpenseVM vm)
        {
           ...
           try
            ...
            vm.CreatedFrom = identity.WorkStationIP;
                ...
                //vm.FiscalYearDetailId = new FiscalYearRepo().FiscalPeriodIdByDate(Ordinary.DateToString(vm.ReimbursableExpenseDate));
          //  vm.ReimbursableExpenseDate = new FiscalYearRepo().FiscalPeriodIdByDate(vm.FiscalYearDetailId);
                ...
                result = new EmployeeReimbursableExpenseRepo().Insert(vm);
                TempData["Msg"] = result[0] + "~" + result[1];
                return RedirectToAction("EmployeeInfoForReimbursableExpense");
        ...
        public ActionResult Edit(string id)
        {
            ...
            EmployeeReimbursableExpenseRepo repo = new EmployeeReimbursableExpenseRepo();
        ...
        [Authorize(Roles = "Master,Admin,Account")]
        [HttpPost]
        public ActionResult Edit(EmployeeReimbursableExpenseVM vm)
        {
            string[] result = new string[6];            
           try
            {  ShampanIdentity identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
            ...
            vm.LastUpdateFrom = identity.WorkStationIP;
                ...
                //vm.FiscalYearDetailId = new FiscalYearRepo().FiscalPeriodIdByDate(Ordinary.DateToString(vm.ReimbursableExpenseDate));
                vm.ReimbursableExpenseDate = new FiscalYearRepo().FiscalPeriodStartDate(vm.FiscalYearDetailId);
                if (vm.FiscalYearDetailId == 0)
                {
                    TempData["Msg"] = "Fail~Fiscal Year Not Exist on this Period";
                    FileLogger.Log(result[0].ToString() + Environment.NewLine + result[2].ToString() + Environment.NewLine + result[5].ToString(), this.GetType().Name, result[4].ToString() + Environment.NewLine + result[3].ToString());
                ...
                result = new EmployeeReimbursableExpenseRepo().Update(vm);
                TempData["Msg"] = result[0] + "~" + result[1];
                return RedirectToAction("Index");
        ...
        [Authorize(Roles = "Master,Admin,Account")]
        public JsonResult EmployeeReimbursableExpenseDelete(string ids)
        {
            EmployeeReimbursableExpenseVM vm = new EmployeeReimbursableExpenseVM();
            ...
            vm.LastUpdateBy = identity.Name;
            vm.LastUpdateFrom = identity.WorkStationIP;
            result = new EmployeeReimbursableExpenseRepo().Delete(vm, a);
            return Json(result[1], JsonRequestBehavior.AllowGet);
        }
        public JsonResult GetPeriodName(string ids)
        {
            var result = _eaRepo.SelectAll().Select(m => m.PeriodName);
            return Json(result, JsonRequestBehavior.AllowGet);
        }

        public ActionResult EmployeeReimbursableExpenseListReport(string view, int? fid = null, string ProjectId = null, string DepartmentId = null, string SectionId = null, string DesignationId = null, string CodeF = null, string CodeT = null, string EmpName = null, string dojFrom = null, string dojTo = null)
        {
            try
            {
                ShampanIdentity identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
                string EmployeeReimbursableExpense = "N";
                ...
                ViewBag.EmployeeReimbursableExpense = EmployeeReimbursableExpense;
                if (view == "Y")
                {
                    return View();
                }
                ...
                List<EmployeeReimbursableExpenseVM> getAllData = new List<EmployeeReimbursableExpenseVM>();
                getAllData = new EmployeeReimbursableExpenseRepo().SelectAll(null, fid);
                ...
                ds.Tables.Add(table);
                ds.Tables[0].TableName = "dtEmpReimbursableExpense";

                string rptLocation = "";
                rptLocation = AppDomain.CurrentDomain.BaseDirectory + @"AllReports\\Payroll\\PayrollEntry\\rptEmployeeReimbursableExpense.rpt";
                ...
                //doc = new rptEmployeeReimbursableExpense();
                //doc.SetDataSource(ds);
