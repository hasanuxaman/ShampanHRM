...
{
    public class EmployeeGFOpeinigController : Controller
    {
        public EmployeeGFOpeinigController()
        {
            ViewBag.TransType = AreaTypeGFVM.TransType;
        }
        EmployeeGFOpeinigRepo _eaRepo = new EmployeeGFOpeinigRepo();
        ...
        public ActionResult _index(JQueryDataTableParamVM param)
        {
            ...
            IEnumerable<EmployeeGFOpeinigVM> filteredData;
            ...
            Func<EmployeeGFOpeinigVM, string> orderingFunction = (c =>
        ...
        public ActionResult SingleEmployeeGFOpeinigEdit(string PFOpeinigId)
        {
            var permission = _reposur.SymRoleSession(identity.UserId, "1_38", "edit").ToString();
            Session["permission"] = permission;
            if (permission == "False")
            ...
            EmployeeGFOpeinigVM empGFOpeinigVM = new EmployeeGFOpeinigVM();
            if (!string.IsNullOrWhiteSpace(PFOpeinigId))
                empGFOpeinigVM = _eaRepo.SelectById(PFOpeinigId);
            EmployeeInfoVM vm = new EmployeeInfoVM();
            EmployeeInfoRepo repo = new EmployeeInfoRepo();
            if (!string.IsNullOrWhiteSpace(PFOpeinigId) && !string.IsNullOrWhiteSpace(empGFOpeinigVM.Id))
            {
                vm = repo.SelectById(empGFOpeinigVM.EmployeeId);
            }
            vm.empGFOpeinigVM = empGFOpeinigVM;
            Session["PFOpeinigId"] = empGFOpeinigVM.Id;
            return View(vm);
        }
        ...
        public ActionResult DetailCreate(string empcode = "", string btn = "current", string id = "0")
        {
            ...
            EmployeeGFOpeinigVM PFOpeinigVM = new EmployeeGFOpeinigVM();
            EmployeeInfoVM vm = new EmployeeInfoVM();
            ...
            else
            {
                ...
                vm.empGFOpeinigVM = PFOpeinigVM;
                vm.empGFOpeinigVM.EmployeeId = EmployeeId;
            }
        ...
        public ActionResult Create(EmployeeInfoVM empVM)
        {
            ...
            try
            {
                ...
                ////return Redirect("/GF/EmployeeBreakMonthPF/SingleEmployeeBreakMonthPFEdit?PFOpeinigId=" + result[2].ToString());
        ...
        public ActionResult MultiplePost(string ids)
        {
            ...
            EmployeeGFOpeinigVM EarningVM = new EmployeeGFOpeinigVM();
        ...
        public ActionResult ImportEmployeeGFOpeinig()
        {
            var permission = _reposur.SymRoleSession(identity.UserId, "1_38", "add").ToString();
            Session["permission"] = permission;
            if (permission == "False")
        ...
        public ActionResult ImportExcel(HttpPostedFileBase file)
        {
            ...
            try
            {
                ...
                Session["result"] = result[0] + "~" + result[1];
                return RedirectToAction("ImportEmployeeGFOpeinig");
            ...
            , string Orderby = null)
        {
            ...
            try
            {
                ...
                string filename = "EmployeeGFOpening" + "-" + DateTime.Now.ToString("yyyyMMdd");
        ...
        public ActionResult reportVeiw(int id)
        {
            try
            {
                ...
                EmployeeGFOpeinigVM EmployeeGFOpeinigVM = new EmployeeGFOpeinigVM();
