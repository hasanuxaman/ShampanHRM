...
{
    public class EmployeeGFForfeitureController : Controller
    {
        public EmployeeGFForfeitureController()
        {
            ViewBag.TransType = AreaTypeGFVM.TransType;
        }
        EmployeeGFForfeitureRepo _eaRepo = new EmployeeGFForfeitureRepo();
        ...
        public ActionResult _index(JQueryDataTableParamVM param)
        {
            ...
            IEnumerable<EmployeeGFForfeitureVM> filteredData;
            ...
            if (codeFilter != "" || EmployeeNameFilter != "" || OpeningValueFilter != "" || OpeningValue1Filter != "" || OpeningDateFilter != "" || PostFilter != "")
            {
                filteredData = filteredData.Where(c =>
                    ...
                    && (OpeningValue1Filter == "" || c.EmployerProfit.ToString().Contains(OpeningValueFilter.ToLower()))
            ...
            Func<EmployeeGFForfeitureVM, string> orderingFunction = (c =>
        ...
        public ActionResult SingleEmployeeForfeitureEdit(string PFOpeinigId, string Operation = "")
        {
            ...
            EmployeeGFForfeitureVM empGFForfeitureVM = new EmployeeGFForfeitureVM();
            if (!string.IsNullOrWhiteSpace(PFOpeinigId))
                empGFForfeitureVM = _eaRepo.SelectByIdAll(PFOpeinigId);
            EmployeeInfoVM vm = new EmployeeInfoVM();
            EmployeeInfoRepo repo = new EmployeeInfoRepo();
            if (!string.IsNullOrWhiteSpace(PFOpeinigId) && !string.IsNullOrWhiteSpace(empGFForfeitureVM.Id))
            {
                vm = repo.AllSelectById(empGFForfeitureVM.EmployeeId);
            }
            vm.empGFForfeitureVM = empGFForfeitureVM;
            vm.Operation = Operation;
            Session["PFOpeinigId"] = empGFForfeitureVM.Id;
            return View(vm);
        }
        ...
        public ActionResult DetailCreate(string empcode = "", string btn = "current", string id = "0", string Operation = "")
        {
            ...
            EmployeeGFForfeitureVM PFOpeinigVM = new EmployeeGFForfeitureVM();
            EmployeeInfoVM vm = new EmployeeInfoVM();
            if (!string.IsNullOrEmpty(Session["PFOpeinigId"] as string) && Session["PFOpeinigId"] as string != "0")
            {
                ...
                vm.empGFForfeitureVM = PFOpeinigVM;
                Session["PFOpeinigId"] = "";
            ...
            else
            {
                ...
                vm.empGFForfeitureVM = PFOpeinigVM;
                vm.empGFForfeitureVM.EmployeeId = EmployeeId;
                vm.Operation = Operation;
                if (vm.Operation.ToLower() == "add")
                {
                    vm.Id = null;
                    vm.empGFForfeitureVM.Id = null;
                    vm.empGFForfeitureVM.EmployerContribution = 0;
                    vm.empGFForfeitureVM.EmployerProfit = 0;
                    vm.empGFForfeitureVM.OpeningDate = null;
                    vm.empGFForfeitureVM.Post = false;
                }
            }
        ...
        public ActionResult MultiplePost(string ids)
        {
            ...
            EmployeeGFForfeitureVM EarningVM = new EmployeeGFForfeitureVM();
            ...
            , string Orderby = null)
        {
            ...
            try
            {
                ...
                string filename = "EmployeeGFForfeiture" + "-" + DateTime.Now.ToString("yyyyMMdd");
        ...
        public ActionResult ReportView(int id)
        {
            try
            {
                ...
                rptLocation = AppDomain.CurrentDomain.BaseDirectory + @"Files\ReportFiles\GF\\rptGFEmployeeForfeiture.rpt";
        ...
        public ActionResult reportVeiw(int id)
        {
            try
            {
                ...
                EmployeeGFForfeitureVM EmployeeGFForfeitureVM = new EmployeeGFForfeitureVM();
                PFReport report = new PFReport();
                EmployeeGFForfeitureVM = _eaRepo.SelectById(id.ToString());
                vm.Id = id;
                vm.Code = EmployeeGFForfeitureVM.Code;
