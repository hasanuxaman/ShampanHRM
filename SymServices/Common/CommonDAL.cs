...
FROM SuperAdministrator";
        ...
        public string SysDBCreate(string Uname, string Pwd, string DBSource)
        {
            ...
            currConn = _dbsqlConnection.GetConnectionSys(); //start
            if (currConn.State != ConnectionState.Open)
            {
                currConn.Open();
            }
...
, id))+1,10))),0) ";
        ...
        public string SelectServerDateTime(SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
...
";
                ...
                while (dr.Read())
                {
                    ServerDateTime = dr["SystemDate"].ToString();
                }
...
WHERE TABLE_NAME = N'" + tableName + "'";
            ...
            , SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            ...
            try
            {
                ...
                if (currConn == null)
                {
                    currConn = _dbsqlConnection.GetConnectionToDo();
                    if (currConn.State != ConnectionState.Open)
                    {
                        currConn.Open();
                    }
...
";
        ...
        public string CodeGenerationGF(string TransactionTypeGroup, string TransactionType, string Date
            , SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Initializ
            ...
            try
            {
                ...
                try
                {
                    ...
                    int transResultCodeGen = 0;
                    TransactionDate = Convert.ToDateTime(Date);
                    year = TransactionDate.ToString("yyyy");

                    string getMaxIdSql = "";
                    getMaxIdSql += "Select id, Prefix Prefix, isnull(LastNumber,0)  +1 INDENTID ";
                    getMaxIdSql += " FROM CodeGenerations";
                    getMaxIdSql +=
                        " WHERE 1=1 AND CYear=@CYear  AND TransactionType=@TransactionType and TransactionTypeGroup=@TransactionTypeGroup";


                    string INDENTID_STR = "";
                    string PrefixStr = "";
                    SqlCommand objMaxIdComm = new SqlCommand(getMaxIdSql, currConn, transaction);
                    objMaxIdComm.Parameters.AddWithValue("@CYear", year);
                    objMaxIdComm.Parameters.AddWithValue("@TransactionType",
                        TransactionType.ToString() ?? Convert.DBNull);
                    objMaxIdComm.Parameters.AddWithValue("@TransactionTypeGroup",
                        TransactionTypeGroup.ToString() ?? Convert.DBNull);
