...
left outer join HRMDB.dbo.ViewEmployeeInformation ve on empPF.EmployeeId=ve.EmployeeId
...
,i.InvestmentRate
,i.InvestmentValue
,0 BankCharge,0 Interest,0 BankExciseDuty, 'New' TransactionType
 from Investments i
...
,i.MaturityDate
,0 InvestmentRate
,i.InvestmentValue,BankCharge,Interest,BankExciseDuty,case when IsEncashed=0 then 'Renew' else 'Encashment' end TransactionType
...
left outer join " + hrmDB + @".dbo.Project p on pf.ProjectId=p.Id
left outer join " + hrmDB + @".dbo.FiscalYearDetail f on pf.FiscalYearDetailId=f.Id
...
left outer join " + hrmDB + @".dbo.FiscalYearDetail fd on i.FiscalYearDetailId=fd.Id
left outer join EnumInvestmentTypes it on i.InvestmentTypeId=it.Id
...
left outer join " + hrmDB + @".dbo.ViewEmployeeInformation e on pfo.EmployeeId=e.EmployeeId
...
left outer join " + hrmDB + @".dbo.ViewEmployeeInformation e on pfo.EmployeeId=e.EmployeeId
left outer join   " + _dbsqlConnection.HRMDB + @".dbo.ViewEmployeeInformation eOld  on pfo.EmployeeId=eold.EmployeeId
...
from PFDetails
left outer join " + hrmDB + @".dbo.FiscalYearDetail fd on PFDetails.FiscalYearDetailId=fd.Id
union all 
...
left outer join " + hrmDB + @".dbo.ViewEmployeeInformation e on pf.EmployeeId=e.EmployeeId
left outer join   " + _dbsqlConnection.HRMDB + @" .dbo.ViewEmployeeInformation eOld  on pf.EmployeeId=eold.EmployeeId
...
--+InterestAmount
from " + hrmDB + @".dbo.EmployeeLoanDetail
where 1=1
...
";
                ...
                {
                    sqlText += @"  And pf.EmployeeId=@EmployeeId";
                }
...
left outer join " + hrmDB + @".dbo.FiscalYearDetail fd on GFEmployeeProvisions.FiscalYearDetailId=fd.Id
union all 
...
from PFDetails pd
left outer join " + hrmDB + @".dbo.ViewEmployeeInformation e on pd.EmployeeId=e.EmployeeId
left outer join   " + _dbsqlConnection.HRMDB + @" .dbo.ViewEmployeeInformation eOld  on pd.EmployeeId=eold.EmployeeId
left outer join " + hrmDB + @".dbo.FiscalYearDetail fd on pd.FiscalYearDetailId=fd.Id
...
,TypeOfReportSL
,GroupTypeShortName
,TypeOfReportShortName from View_NetChange
...
";
                ...
                {
                    sqlText += @" ,case when c.COAType in('Revenue','Liability','OwnersEquity','RetainedEarnings') then -1*isnull(op.TransactionAmount,0) else isnull(op.TransactionAmount,0) end OpeningAmount 
...
drop table #TempNetProfit";
        ...
        public DataSet IFRSReports(PFReportVM vm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            ...
            try
            {
                vm.HRMDB = _dbsqlConnection.HRMDB;
        ...
        public DataTable InvestmentAccruedSummery()
        {
            ...
            try
            {
                ...
                sqlText = @" 
                        ...
                        ,b.Name [Bank Name]
                        ,n.InvestmentDate [Investment Date]
                        ,n.FromDate [From Date]
                        ,n.ToDate [To Date]
                        ,n.MaturityDate [Maturity Date]
                        ,n.AitInterest as AIT
                        ,fd.PeriodName [Period Name] 
                        ,ia.ReferenceNo [Reference No]
                        ,ia.InvestmentValue [Investment Value]
                        ,ia.AccruedMonth [Accrued Month]
                        ,ia.InterestRate [Interest Rate]
                        ,ia.AccruedInterest [Accrued Interest]
                        ,ia.AitInterest [AIT]
                        ,ia.NetInterest [Net Interest]
                         from InvestmentAccrued ia
...
SELECT 
    @PeriodName = UPPER(LEFT(PeriodName, 3)), 
    @PeriodYear = [Year]
...
FROM 
    View_TIBSalary s
LEFT OUTER JOIN 
    ViewEmployeeInformation e ON s.EmployeeId = e.EmployeeId
LEFT OUTER JOIN 
    EmployeePersonalDetail p ON s.EmployeeId = p.EmployeeId
CROSS APPLY (
    VALUES (
...
WHERE 
  ...
  Select 'CN0001' AS [Account code],
    ...
    SUM(-[Transaction Amount]) AS [Base Amount],
    NULL AS [BRANCHE T1],
    NULL AS [ACTIVITY T2],
...
";
        ...
        WITH cat AS (
            SELECT
                ...
                'P-EC-2023 ' + e.EmpName AS Description,
                'PROFIT TF 2023' AS [Transaction Reference],
                e.Code AS [Extra Reference],
                '' AS [Transaction Date],
                'BDT' AS [Currency Code],
                s.TotalProfit * -1 AS [Transaction Amount],
                s.TotalProfit * -1 AS [Base Amount],
                e.Branch AS [BRANCHE T1],
                e.Department AS [ACTIVITY T2],
                e.TPNCode AS [CLIENT/SUPPLIER T3],
                '' AS [VAT TAX],
                e.DCode AS [EMPLOYEE CODE T5],
                '' AS [JOB FILE T6],
                ...
                '' AS [T9]
            FROM 
                 " + _dbsqlConnection.PFDB + @" .[dbo].[ProfitDistributionNew] s
            LEFT OUTER JOIN 
                ViewEmployeeInformation e ON s.EmployeeId = e.EmployeeId
            WHERE 
            ...
            SELECT
                ...
                'P-CC-2023' + e.EmpName AS Description,
                'PROFIT TF 2023' AS [Transaction Reference],
                e.Code AS [Extra Reference],
                '' AS [Transaction Date],
                'BDT' AS [Currency Code],
        ...
        SELECT 
            ...
            'TPROFIT TF-2023' AS Description,
            'TPROFIT TF-2023' AS [Transaction Reference],
            NULL AS [Extra Reference],
            NULL AS [Transaction Date],
            NULL AS [Currency Code],
            SUM([Transaction Amount]) AS [Transaction Amount],
